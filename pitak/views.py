...
viloyat = ReplyKeyboardMarkup([[KeyboardButton('Toshkent'), KeyboardButton('Samarqand')],
                              [KeyboardButton('Sirdaryo'), KeyboardButton('Jizzax')],
                              [KeyboardButton('Farg\'ona'), KeyboardButton('Namangan')],
                              [KeyboardButton('Andijon'), KeyboardButton('Qashqadaryo')],
                              [KeyboardButton('Surxondaryo'), KeyboardButton('Buxoro')],
                              [KeyboardButton('Navoiy'), KeyboardButton('Xorazim')],
                              [KeyboardButton('Nukus')],
                              [KeyboardButton('🔙Ortga'), KeyboardButton('🏠Bosh sahifa')]],
...
def order(update: Update, context):
    ...
    #     update.message.reply_text("Shafyormisiz yoki Yo'lovchi!",
    #                               reply_markup=ReplyKeyboardMarkup([[KeyboardButton('Shafyor'), KeyboardButton("Yo'lovchi")]],
    #                                                                resize_keyboard=True, one_time_keyboard=True))
    #     Users.objects.filter(id=user_id).update(step=1)
    # elif step.step == 1 and msg != '🔙Ortga' and msg != '🏠Bosh sahifa' and msg=='Shafyor':
    if step.step == 0 and msg == "🔛Boshlash":
        update.message.reply_text("Avtomabil turini yozing!")
        # Users.objects.filter(id=user_id).update(status=msg)
        Users.objects.filter(id=user_id).update(step=2)
    # elif step.step == 1 and msg != '🔙Ortga' and msg != '🏠Bosh sahifa' and msg=="Yo'lovchi":
    #     update.message.reply_text("Qanaqa avtomobilda bormoqchisiz!Yoki farqi yo'q!", reply_markup=back)
    #     Users.objects.filter(id=user_id).update(status=msg)
    #     Users.objects.filter(id=user_id).update(step=2)
    # elif step.step==2 and msg == '🔙Ortga':
    #     update.message.reply_text("Shafyormisiz yoki Yo'lovchi!",
    #                               reply_markup=ReplyKeyboardMarkup(
    #                                   [[KeyboardButton('Shafyor'), KeyboardButton("Yo'lovchi")]],
    #                                   resize_keyboard=True, one_time_keyboard=True))
    #     Users.objects.filter(id=user_id).update(step=1)
        ...
        # else:
        #     update.message.reply_text("Qanaqa avtomobilda bormoqchisiz!Yoki farqi yo'q!", reply_markup=back)
        update.message.reply_text("Avtomabil turini yozing!", reply_markup=back)
        Users.objects.filter(id=user_id).update(step=2)
    elif step.step == 3 and msg != '🔙Ortga' and msg != '🏠Bosh sahifa' and msg:
        update.message.reply_text("Qayerga ketish joy(lar)ini kiriting!", reply_markup=viloyat)
        ...
        Users.objects.filter(id=user_id).update(step=7)
    elif step.step == 6 and phone:
        update.message.reply_text("Qisqa izoh yozishingiz mumkin", reply_markup=ReplyKeyboardMarkup([[KeyboardButton('Izoh yo\'q')],
                                                                                                     [KeyboardButton('🔙Ortga'), KeyboardButton('🏠Bosh sahifa')]],
                                                                                                    resize_keyboard=True, one_time_keyboard=True))
        Users.objects.filter(id=user_id).update(phone=phone.phone_number)
        Users.objects.filter(id=user_id).update(step=7)
    elif step.step == 7 and msg == '🔙Ortga':
        Users.objects.filter(id=user_id).update(phone=msg)
        Users.objects.filter(id=user_id).update(step=5)
        update.message.reply_text("Telefon raqamni yozing yoki pasdagi tugmani bosing!",
                                  reply_markup=ReplyKeyboardMarkup([[KeyboardButton("☎Telefon raqamni yuborish",
                                                                   ...
                                                                   resize_keyboard=True,
                                                                   one_time_keyboard=True))
    elif step.step == 7 and msg != '🔙Ortga' and msg != '🏠Bosh sahifa' and msg:
        ...
        if Users.objects.get(id=user_id).comment != 'Izoh yo\'q':
            text += f"🔎Izoh: {obj.comment}\n"
        text += "Sizning e'loningiz @onlin_bozor_m kanaliga yuboriladi."
...
def inline(update: Update, context: CallbackContext):
    ...
    if data == "send":
        ...
        context.bot.send_message(chat_id=update.callback_query.from_user.id,
                                 text="✅Buyurma @onlin_bozor_m kanalga yuborildi!, Yangi buyurtma qilishingiz mumkin",
